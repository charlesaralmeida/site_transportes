...
const OSReport = () => {
        ...
        .then((result) => {
          if (result) {
            ...
            for (let os in result) {
              ...
              Firebase.updateData(path, data).catch((error) =>
                dispatch(showWarning({ title: "Erro", text: error.message }))
              );
            }
            Firebase.readData(PATH_OS_PROCESSED_LIST).then((list) => {
              if (list) list = list.concat(newlist);
              else list = newlist;
              Firebase.writeData(PATH_OS_PROCESSED_LIST, list).then(() => {
                list.forEach((authCode) => {
                  let path = PATH_OS_VALIDATED + "/" + authCode;
                  Firebase.deleteData(path).then(() =>
  ...
  const getDataToTable = () => {
    ...
    if (authCodes.length) {
      ...
      const rows = Object.keys(adminState.OSData).map(
        (key) => adminState.OSData[key]
      );
      return { headers, rows };
    }
          ...
          <div className="content-admin-os-validadas-container content-admin-container">
            ...
            <div className="content-admin-btn-container">
              <button
                ...
                onClick={exportOsToExcel(PATH_OS_NOT_VALIDATED)}
              >
                Exportar
              </button>
            </div>
          </div>
          {adminState.isDataReady
            ? makeTable(getDataToTable().headers, getDataToTable().rows)
            : null}
          <div className="content-admin-os-validadas-container content-admin-container">
            <h1>OS validadas</h1>
            <br />
            <div className="content-admin-btn-container">
              <button
                className="button-admin"
                onClick={exportOsToExcel(PATH_OS_VALIDATED)}
              >
                Exportar
              </button>
              <button className="button-admin" onClick={processAllOs}>
                Processar
              </button>
            </div>
          </div>
