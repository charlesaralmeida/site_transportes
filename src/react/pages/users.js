...
const Users = () => {
  ...
  const MSG = {
    ...
    CHECK_ERRORS: {
      title: "Atenção!",
      text: "Verifique os erros de preenchimento",
    },
    SAVE_SUCCESS: { title: "Atenção!", text: "Dados salvos com sucesso" },
    MAIL_ERROR:
      "O e-mail do usuário deve estar no formato 'usuário@unicamp.br'",
  };
  ...
  const handleProfileChoice = (value) => {
    ...
    if (value) {
      toggleLoader(true);
      Firebase.readData(ORGAOS_LISTA_PATH).then((lista) => {
  ...
  const handleCodChoice = (e) => {
    ...
    if (value) {
      toggleLoader(true);
      Firebase.readData(
        ORGAOS_PATH + "/" + unmaskCodOrgao(value) + "/0/Nome"
      ).then((nomeUnidade) => {
        setCodSelected(true);
    ...
    } else setCodSelected(false);
    removeError(e.target.name);
  };

  const setUserInfo = (e) => {
    if (!e.target.value) toggleUserChanged(false);
  ...
  const removeError = (inputName) => {
    ...
    if (errors.hasOwnProperty(inputName)) delete errors[inputName];
    setError(errors);
  };
      ...
      .then(() => {
        Firebase.readData(USERS_ADMIN_LISTA_PATH)
          .then((lista) => {
            ...
            Firebase.writeData(USERS_ADMIN_LISTA_PATH, lista).then(() => {
              toggleUserChanged(false);
              toggleLoader(false);
              dispatch(showWarning(MSG.SAVE_SUCCESS));
              let path =
                USERS_UNIDADE_LISTA_PATH + "/" + unmaskCodOrgao(userCodUnidade);
              Firebase.readData(path)
                .then((lista) => {
                  ...
                  Firebase.writeData(path, lista)
                    ...
                    .catch((error) => {
                      dispatch(
                        showWarning({ title: "Atenção!", text: error.message })
                      );
                      toggleUserChanged(false);
                      toggleLoader(false);
                    });
        ...
        <form className="form-fornecedor" onSubmit={(e) => formSubmit(e)}>
          ...
          <div className="form-fornecedor-dados">
            <div className="form-fornecedor-select-container">
              ...
              <select
                className="form-fornecedor-select"
                onChange={(e) => handleProfileChoice(e.target.value)}
                value={isProfileSelected ? userProfile : ""}
              >
              ...
              <div className="tooltip">
                <p>
                  <strong>Admin:</strong> Acesso irrestrito a todas as
                  ferramentas
                </p>
                <p>
                  <strong>Solicitante:</strong> usuário habilitado para
                  solicitar viagens
                </p>
                <p>
                  <strong>Expedidor:</strong> principal responsável pela
                  comunicação entre a DGA e a Unidade/Órgão
                </p>
                <p>
                  <strong>Gestor:</strong> usuário com acesso a relatórios e
                  cadastros de uma unidade específica
                </p>
              </div>
            </div>
            {isProfileSelected ? (
              <div>
                <div className="form-fornecedor-input-container">
                  ...
                  <input
                    className="form-fornecedor-input"
                    name="nome"
                    value={userName}
                ...
                <div className="form-fornecedor-input-container">
                  ...
                  <input
                    ...
                    onChange={(e) => setUserInfo(e)}
                  />
                  <p className="input-fornecedor-error">
                    {errorsForm["ramal"]}
                  </p>
                </div>
                <div className="form-fornecedor-select-container">
                  <span className="form-fornecedor-select-label">
                    Cod. Unidade/Órgão
                  </span>
                  <select
                    name="unidade"
                    className="form-fornecedor-select"
                    onChange={(e) => handleCodChoice(e)}
                    value={isCodSelected ? userCodUnidade : ""}
                  >
                    <option value="">Selecione o código</option>
                    {unidadesList.map((cod, index) => {
                      cod = maskCodOrgao(cod);
                      return (
                        <option key={index} value={cod}>
                          {cod}
                        </option>
                      );
                    })}
            ...
            <ButtonAddUserContainer isUserChanged={isUserChanged} />
          </div>
        </form>
      ) : null}
    </>
